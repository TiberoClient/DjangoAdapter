...
class DatabaseCreation(BaseDatabaseCreation):
    ...
    def _switch_to_test_user(self, parameters):
        """
        Tibero doesn't have the concept of separate databases under the same user.
    ...
    def _execute_test_db_creation(self, cursor, parameters, verbosity, keepdb=False):
        ...
        acceptable_err =  None
        self._execute_allow_fail_statements(cursor, statements, parameters, verbosity, acceptable_err)
    ...
    def _create_test_user(self, cursor, parameters, verbosity, keepdb=False):
        ...
        success = self._execute_allow_fail_statements(cursor, [extra], parameters, verbosity, None)
    ...
    def _execute_statements(self, cursor, statements, parameters, verbosity, allow_quiet_fail=False):
        for template in statements:
            ...
            try:
                ...
                if a(not allow_quiet_fail) or verbosity >= 2:
                    sys.stderr.write("Failed (%s)\n" % (err))
                raise

    def _execute_allow_fail_statements(self, cursor, statements, parameters, verbosity, acceptable_err):
        """
        Execute statements which are allowed to fail silently if the Tibero
        error code given by `acceptable_err` is raised. Return True if the
        statements execute without an exception, or False otherwise.
        """
        try:
            # Statement can fail when acceptable_err is not None
            allow_quiet_fail = acceptable_err is not None and len(acceptable_err) > 0
            self._execute_statements(cursor, statements, parameters, verbosity, allow_quiet_fail=allow_quiet_fail)
            return True
        except DatabaseError as err:
            description = str(err)
            if acceptable_err is None or acceptable_err not in description:
                raise
            return False
    ...
    def _get_test_db_name(self):
        ...
        names as handled by Django haven't real counterparts in Tibero.
        """
        return self.connection.settings_dict['NAME']
