...
class DatabaseSchemaEditor(BaseDatabaseSchemaEditor):
    ...
    def _digest(cls, *args):
        ...
        h.update(force_bytes(datetime.datetime.now()))
    ...
    def quote_value(self, value):
        if isinstance(value, (datetime.date, datetime.time, datetime.datetime)):
            ...
            b = value.days
            c = int(value.seconds/3600)
            d = int((value.seconds-c*3600)/60)
            e = value.seconds-c*3600-d*60
            return "'"+str(b)+' '+str(c)+':'+str(d)+':'+str(e)+'.'+str(value.microseconds)+"'"
        else:
            return str(value)
